...
namespace Barebones.Networking {
    ...
    public class ServerSocketWs : IServerSocket, IUpdatable {
        private readonly Queue<Action> _executeOnUpdate;

        private readonly float _initialDelay = 0;
        ...
        public void Update() {
            ...
            lock (_executeOnUpdate) {
                while (_executeOnUpdate.Count > 0) _executeOnUpdate.Dequeue().Invoke();
            }
        }
        ...
        private void SetupService(WebSocketServer server) {
            server.AddWebSocketService("/w", () => {
                var service = new WsService(this);
                var peer = new PeerWsServer(service);

                service.OnMessageEvent += data => { peer.HandleDataReceived(data, 0); };
                ...
                service.OnCloseEvent += reason => { peer.NotifyDisconnectEvent(); };
