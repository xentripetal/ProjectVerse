...
public class ChatUi : MonoBehaviour {
    ...
    /// </summary>
    public string[] AutoJoinChannels = {"Global"};

    /// <summary>
    ///     List of available chat window sizes
    /// </summary>
    public int[] AvailableSizes = {150, 200, 250, 300};
    ...
    /// </summary>
    public string[] ChannelMasks = {"Game-"};
    ...
    [Header("Components")] public Image MessagesBg;
    ...
    protected virtual void Awake() {
        ...
        InputField.onEndEdit.AddListener(val => {
            if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter)) OnSendClick();
        });
    ...
    protected virtual void OnLoggedIn() {
        ...
        foreach (var channel in AutoJoinChannels) {
            ...
            Msf.Client.Chat.JoinChannel(channel, (successful, error) => {
                if (successful) PushInfoMessage(string.Format(ChannelJoinedMessage, channelName));
            });
        }
    }
    ...
    protected virtual void PushChannelMessage(ChatMessagePacket packet) {
        ...
        if (ChannelMasks.Any(c => receiver.Contains(c))) packet.Receiver = "";
    ...
    protected Text GetTextObject() {
        var text = _currentMessages.Count >= MaxMessages ? _currentMessages.Dequeue() : Instantiate(MessagePrefab);
    ...
    public virtual void OnToggleBgClick() {
        MessagesBg.color = MessagesBg.color.a < 0.1f ? _bgColor : new Color(0, 0, 0, 0.01f);
    }
