...
namespace Barebones.MasterServer {
    public class LobbyUi : MonoBehaviour, ILobbyListener {
        ...
        public virtual void Setup(JoinedLobby lobby) {
            ...
            // Setup teams
            foreach (var team in lobby.Teams) Teams.Add(team.Key, CreateTeamView(team.Key, team.Value));
        ...
        protected virtual string GenerateUsernameText(LobbyUserUi view) {
            ...
            if (view.IsCurrentPlayer) username += CurrentPlayerText;

            if (view.IsMaster) username += MasterText;

            return username;
        }

        protected virtual void OnDestroy() {
            if (JoinedLobby != null && JoinedLobby.Listener.Equals(this)) JoinedLobby.SetListener(null);
        }
        ...
        protected virtual void UpdateTeamJoinButtons() {
            ...
            foreach (var team in Teams) {
                ...
                if (team.Key == currentPlayer.Team) team.Value.JoinButton.gameObject.SetActive(false);
            }
        }
        ...
        protected virtual void UpdatePlayerStates() {
            ...
            foreach (var userView in Users.Values) userView.SetReadyStatusVisibility(showReadyState);
        }
