...
using UnityEngine;
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/Drag Vertex")]
public class UIDragVertex : MonoBehaviour
{
	...
	protected void Move (Vector3 worldDelta)
	{
		if (panelRegion != null) {
			...
			if (parent != null) {
				...
				if (rb != null) {
					...
					if (restrictWithinPanel && dragEffect != DragEffect.MomentumAndSpring)
						Debug.LogWarning(
							"Constraining doesn't work properly when there is a rigidbody present because rigidbodies move in FixedUpdate, not Update.\n" +
							"Please remove it, or use the MomentumAndSpring type drag effect.", rb);
#endif
				}
				else target.localPosition = RoundPositionToPixel(after);
			}
			else if (rb != null) {
				rb.position = RoundPositionToPixel(mTargetPos);
			}
			else target.position = RoundPositionToPixel(mTargetPos);

			UIScrollView ds = panelRegion.GetComponent<UIScrollView>();
			if (ds != null) ds.UpdateScrollbars(true);
		}
		else target.position = RoundPositionToPixel(target.position + worldDelta);
	}

	private Vector3 RoundPositionToPixel(Vector3 targetPos) {
		targetPos = new Vector3(Mathf.Round(targetPos.x / PixelSize) * PixelSize,
			Mathf.Round(targetPos.y / PixelSize) * PixelSize, targetPos.z);
		return targetPos;
	} 
